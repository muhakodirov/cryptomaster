...
function News() {
  ...
  console.log('2024-03-25' === selectedDate) //true
  ...
  const filteredOption = selectedValue ? currentData.data?.filter(item => item.attributes?.title.includes(selectedValue)) : currentData?.data
  const filteredData = selectedDate ? filteredOption.filter(item => !(moment(item.attributes?.publishOn).format('YYYY-MM-DD') != selectedDate)) : filteredOption
  const filteredNews = filteredData
    ...
    <div className={styles.container}>
      ...
      <div className={styles.inputContainer}>
        <div className={styles.selectInput}>
          ...
          <input onChange={(e) => setSelectedDate(e.target.value)} id='input' title='Filter news by time' type="date" />
        </div>
      </div>
      <div className={styles.newsContainer}>
        ...
        <div className={styles.flexContainer}>
          {(isLoading || currentData?.data.length === undefined) && <Stack
            paddingTop={20}
            // paddingLeft={50}     
            alignItems="center"
          ...
          {!(filteredNews?.length <= 0) ? filteredNews?.map((item, id) => {
            ...
            : <Stack sx={{ width: '100%' }} spacing={2}>
              <Alert variant="filled" severity="error"> No news for this filter </Alert>
            </Stack>}
        </div>
